...
- 知识点选择。
- 难度和分值编辑。
- AI 标准化生成候选题干，展示候选源码和候选预览，用户应用后再保存。
- AI 解析草稿生成。
- 保存校验结果。
- 单题入库和批量入库。
...
- 复合编辑器必须在每个可编辑小问上保留独立 `AI 标准化` 按钮。小问级标准化只处理当前小问的 `manualMarkdown`，展示候选源码/预览，用户点击应用后只更新该小问的题干、答案、解析或选项；不得覆盖父题材料、其它小问或题库原题的试卷层小问选择。
...
- 题目保存和入库时必须保留 `images` 数组；历史 Markdown 图片引用需要归一到题图模块，避免重复渲染。
- 已有图片引用不得重复插入。
...
- 优先生成 Markdown + LaTeX 中间文件。
- DOCX 优先通过 Pandoc 生成。
- PDF 优先通过 Pandoc + XeLaTeX 生成。
- 公式应尽量转为 Word 原生公式或稳定 PDF 渲染。
- 题图必须进入导出文件。
- `tasks` 环境在导出前转为普通 Markdown 列表。
- Pandoc 或 XeLaTeX 不可用时，返回明确错误或使用旧导出 fallback。
...
导出链路通过 Markdown + LaTeX 中间文件生成 DOCX/PDF。后端优先调用 Pandoc 转换为 DOCX/PDF；公式保持原始 LaTeX 数学语法，DOCX 中由 Pandoc 转为 Word 原生公式，PDF 由 XeLaTeX 渲染。Pandoc 或 XeLaTeX 不可用时回退旧导出。
