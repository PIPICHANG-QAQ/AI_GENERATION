...
6. `visual-repair`：节点仍排在边界确认和结构构建之后，读取最终题目结构与 OCR bbox 裁出题目 crop。本节点内部按题目有界并发，执行本地长横线检测和可选 Pix2Text 二次 OCR；每个线程只产出单题修复结果，主线程按原始题号顺序合并，避免视觉修复影响题目边界识别。
