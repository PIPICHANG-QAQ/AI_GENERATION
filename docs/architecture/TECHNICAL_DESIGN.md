...
flowchart TD
  ...
  RuleSplit["本地规则拆题参考<br/>题号、题型粗判、选项、题图归属"]:::local

  LlmSplit{"大模型拆题是否启用且可用？"}:::mixed
  SplitApi["外部 API：大模型结构化拆题<br/>输入 OCR Markdown、图片资源、本地参考"]:::external
  ...
  SplitNormalize --> StructureProtect
  RuleFallback --> StructureProtect
...
- 提供 `/api/capabilities/export-flow/runtime`，代理 Python worker 的 Pandoc、XeLaTeX、中文字体和导出格式运行时状态。
...
- 接管导入题和题库题 AI 标准化/AI 解析入口，由 Java 记录 AI job、调用 Python worker；AI 标准化默认返回候选，前端人工应用后再保存，显式写回请求仍需通过低置信和严重 LaTeX 风险闸门；AI 解析返回答案/解析时自动写回题目。
...
- 不把 OCR provider、大模型、题图处理、Pandoc 导出逻辑从 Python 中移除；Python 已提供 `/worker/*` 内部入口，旧 `/api/*` 仍保留。
...
试卷导出主链路为 Markdown + Pandoc：

1. 后端先在 `backend/storage/exports/{paperId}.md` 生成 Markdown 中间文件。
2. 中间文件保留 `$...$` / `$$...$$` 数学公式，标题、卷头、题目、答案和解析都写入 Markdown。
3. 题图解析为本地绝对路径并写成 Markdown 图片；LaTeX `tasks` 环境在导出前转成普通 Markdown 列表，避免非标准环境影响 Pandoc。
4. DOCX 使用 `pandoc paper.md -o paper.docx`，公式由 Pandoc 转成 Word 原生 OMML 公式对象。
5. PDF 使用 `pandoc paper.md --pdf-engine=xelatex -o paper.pdf`，中文字体通过 `PANDOC_CJK_FONT` 配置，默认尝试 `Songti SC`。
6. 如果 Pandoc 或 XeLaTeX 不可用，后端回退旧的 `python-docx` / `reportlab` 导出，保证接口仍可返回文件。
...
- 任务超时：任务失败，并返回超时信息。
- 大模型未配置、关闭、超时、接口失败或返回非法 JSON：不影响 OCR 任务成功，返回本地规则拆题结果，并在 `splitter.error` 中记录兜底原因。
...
大模型拆题逻辑位于 `backend/python-worker/app/llm_splitter.py`。后端在 `collect_outputs` 阶段执行证据驱动拆题流水线：

1. `local-boundary-detect`：本地规则只识别大题标题、题号、小问、A/B/C/D 选项和题图候选边界。
2. `llm-boundary-refine`：OpenAI 兼容大模型只确认或修正 `start/end`、`label`、`path` 等边界字段，禁止生成题干、答案或解析。
...
6. `visual-repair`：读取 OCR bbox 裁出题目 crop，本地检测长横线；Pix2Text 配置可用时只对低置信题目 crop 做二次 OCR，并在安全校验通过后补回题干。
...
- `stemMarkdown` 或 `manualMarkdown` 只保存题干文字、公式和选项 Markdown，不再补写题图 Markdown 引用。
- 前端在题干下方以独立“题图”模块渲染 `question.images`，并支持从任务题图库选择或本地多图上传。
- 题库中心编辑弹窗复用同一题图模块；已入库题目可继续从源导入任务图片库选择题图，也可上传题库题目专属图片。
- 后端保存人工校验结果和导入题目入库时会清理历史 Markdown 中的题图引用，避免同一图片在题干和题图模块重复出现。
...
2. OCR 结果返回前，后端对本地校验命中的语义 OCR 风险自动执行一次 AI 修复。
...
6. 自动修复写回后会重新执行本地公式标准化与校验，保证 `mathValidation` 与实际题干一致。
...
8. 如果大模型已配置，后端会在 OCR 结果返回前自动尝试修复第 7 类语义 OCR 风险。
9. 人工保存后的 `manualMarkdown` 会优先进入公式标准化与校验。
10. 读取历史 OCR 任务时，如果发现旧版“连续 $$”告警，会自动重新生成结构化结果并写回任务 JSON。
11. 人工触发 AI 标准化时，后端会额外执行严重 LaTeX 损坏检测，并在导入题/题库题上下文中向模型提供原始 OCR 片段，辅助修复渲染完全失败的公式；当原始 OCR 片段已经比当前编辑稿更可靠时，会直接作为候选返回。
...
后端在 `collect_outputs` 阶段生成结构化题目数据。当前策略是“大模型优先，本地规则兜底”。本地规则优先使用 MinerU 的 `*_content_list_v2.json`，并作为大模型输入参考。
...
7. 导入任务和题库中心展示题目时，题图必须跟随题干区域展示，但由 `images` 数组驱动，不能依赖题干 Markdown 图片引用。
