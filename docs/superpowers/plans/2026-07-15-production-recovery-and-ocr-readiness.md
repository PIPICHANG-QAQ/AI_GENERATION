...
Expected: compare 为 equal、differenceCount 为 0。这里只验证确定性 replay 工具和 corpus 可用性，不冒充正式受控 benchmark。
...
- Server staging: `/home/user/releases/AI_GENERATION_TOGO_20260715_PRODUCTION_RECOVERY/`
- Server backup pattern: `/home/user/backups/AI_GENERATION_DOCKER_{UTC时间戳}/`
...
```bash
ssh -p 3322 user@120.211.112.121 'mkdir -p /home/user/releases/AI_GENERATION_TOGO_20260715_PRODUCTION_RECOVERY'
scp -P 3322 \
  dist/AI_GENERATION_TOGO_20260715_PRODUCTION_RECOVERY.tar.gz \
  dist/AI_GENERATION_TOGO_20260715_PRODUCTION_RECOVERY_MANIFEST.json \
  user@120.211.112.121:/home/user/releases/AI_GENERATION_TOGO_20260715_PRODUCTION_RECOVERY/
```
...
release=/home/user/releases/AI_GENERATION_TOGO_20260715_PRODUCTION_RECOVERY
stamp="$(date -u +%Y%m%dT%H%M%SZ)"
backup="/home/user/backups/AI_GENERATION_DOCKER_${stamp}"
mkdir -p "$backup"
cp -a .env "$backup/.env"
