...
Capability-specific APIs such as review-workbench image libraries, file-flow image selection, AI standardize writeback, and callback events are part of the generated SDK surface. Export jobs and lower-level runtime diagnostics remain discoverable through `GET /api/capabilities` and can be expanded in SDK packages as platform usage stabilizes.
