...
当前 OpenAPI 契约版本：`1.2.0`。相对 `1.1.0` 的变化是非破坏性 MINOR 升级：OCR-Flow 新增顶层类型化 capability descriptor，`providerContract`、`postProcessContract` 仍是可扩展契约 Map；TypeScript/Java SDK 新增 `getOcrFlowCapability()`，已有接口、`rescanImportTask(jobId)` 和 `question-package.v1` 结构保持兼容。
